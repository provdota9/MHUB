...
makefolder('Skele Hub\\Anime Adventures')


local DefaultFiles = {

	['Skele Hub\\Settings_' .. player.Name] = {
		...
		['PortalUSE Start After Time'] = 10;
		['PortalUSE Star After Players'] = 2;
...
local Settings = HttpService:JSONDecode(readfile('Skele Hub\\Settings_' .. player.Name)) 

local function Save (valueName, newValue)
	Settings[valueName] = newValue
	writefile('Skele Hub\\Settings_' .. player.Name, HttpService:JSONEncode(Settings))
end

local function GetSave (valueName)
	local value = Settings[valueName]
	if value == nil then
		if DefaultFiles['Skele Hub\\Settings_' .. player.Name][valueName] ~= nil then
			Save(valueName, DefaultFiles['Skele Hub\\Settings_' .. player.Name][valueName])
		else
...
local macroMapList = {
	...
	['Tower'] = {'Thriller Park', 'Entertainment District', 'West City', 'Rain Village', 'Devil City', 'Bizzare Town', 'Karakora Town', 'Storm Hideout', 'Infinity Train',};
...
Raid_SelectWorld.MouseButton1Click:Connect(function()
	...
	for _, worldModule in ipairs(RS.src.Data.Worlds:GetChildren()) do
		...
		for _,worldAbout in pairs(require(worldModule)) do
			if worldAbout.name ~= 'Hero City (Hero Slayer)' and worldAbout.name ~= 'Sacred Planet' and worldAbout.name ~= 'Bizzare Town' and worldAbout.name ~= '' and worldAbout.name ~= "Marine's Ford (Buddha)" and worldAbout.name ~= 'Entertainment District' and worldAbout.name ~= "West City (Freezo's Invasion)" and worldAbout.name ~= 'West City' and worldAbout.name ~= 'Storm Hideout' and worldAbout.name ~= 'Infinity Train' and worldAbout.name ~= 'Hidden Sand Village' and worldAbout.name ~= 'Shiganshinu District' then continue end
...
AutoPlaceWave.FocusLost:Connect(function()
	if AutoPlaceWave.Text == '' or not t    onumber(AutoPlaceWave.Text) then AutoPlaceWave.Text = GetSave('AutoPlaceUnitsWave') return end
...
for tabName, mapsList in pairs(macroMapList) do
	...
	for _, mapName in ipairs(mapsList) do
		...
		newDDLMacroMap.MouseButton1Click:Connect(function()
			...
			local MacrosList = listfiles('Skele Hub\\Anime Adventures')
...
local function RecordMacroFunc ()
	...
	local macroExist = isfile('Skele Hub\\Anime Adventures\\' .. MacroFile )
	local macroExistJson = isfile('Skele Hub\\Anime Adventures\\' .. MacroFile .. '.json')

	if macroExistJson then MacroFile = 'Skele Hub\\Anime Adventures\\' .. MacroFile .. '.json'
	elseif macroExist then MacroFile = 'Skele Hub\\Anime Adventures\\' .. MacroFile
...
createMacroBox.FocusLost:Connect(function(enterPressed)
	...
	writefile('Skele Hub\\Anime Adventures\\' .. createMacroBox.Text .. '.json', HttpService:JSONEncode({}))
end)
...
deleteMacroButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Skele Hub\\Anime Adventures\\' .. item)
	local macroExistjson = isfile('Skele Hub\\Anime Adventures\\' .. item .. '.json')
	if macroExistjson then item = item .. '.json' end

	if not macroExist and not macroExistjson then return end

	delfile('Skele Hub\\Anime Adventures\\' .. item)
...
equipMacroUnitsButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Skele Hub\\Anime Adventures\\' .. macroSelected)
	local macroExistjson = isfile('Skele Hub\\Anime Adventures\\' .. macroSelected .. '.json')
	if macroExistjson then macroSelected = macroSelected end

	if not macroExist and not macroExistjson then equippingMacroUnits = false return end

	local MacroAbout = HttpService:JSONDecode( readfile( 'Skele Hub\\Anime Adventures\\' .. macroSelected .. '.json') )
...
function HideTabFunc (enabled)
	...
	local userNameNew = 'Skele Hub ON TOP'
...
local function webhook ()
	...
	local TotalDamage = makeComma(math.floor(player._stats.damage_dealt.Value + 0.5))
	local BattlePass = ''
	local reachedTier = 0
		...
		["embeds"] = {

			{
				...
				['fields'] = {
					{
						['name'] = "Player Stats",
						['value'] = string.format( "<:Gems:1148368507029950515> %s\n<:Gold:1148368511463338074> %s\n<:Candy:1179714718613651456> %s\n:tickets: Tier: %s\n :broken_heart:Kills: %s\n :crossed_swords:Damage: %s", TotalGems, TotalGold, TotalCandy, BattlePass, TotalKills, TotalDamage),
						['inline'] = true
					},
...
task.spawn(function() 
	pcall(function()
		...
		if queue_on_teleport then
			local SkeleHubSCRIPT = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/provdota9/MHUB/main/Alpha.lua"))()'
